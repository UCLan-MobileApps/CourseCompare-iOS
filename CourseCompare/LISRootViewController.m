...
-(void)pickerReply:(NSInteger)pickNumber:(NSString *)pickerString:(NSString *)pickerName {
    ...
    if (pickNumber == 1) {
        [institutions setTitle:pickerName forState:UIControlStateNormal];
        providerString = pickerString;
    }
    if (pickNumber == 2) {
    ...
    if (pickNumber == 4) {
        [searchdistance setTitle:pickerName forState:UIControlStateNormal];
        distString = pickerString;
    }
    if (pickNumber == 5) {
        [displayorder setTitle:pickerName forState:UIControlStateNormal];
        orderString = pickerString;
    }
    if (pickNumber == 6) {
        [units setTitle:pickerName forState:UIControlStateNormal];
        unitString = pickerString;
    }
}
...
-(NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (pickerNumber == 1) {
        return [[providerArray objectAtIndex:row] objectForKey:@"Name"];
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (pickerNumber == 1) {
        [institutions setTitle:[[providerArray objectAtIndex:row] objectForKey:@"Name"] forState:UIControlStateNormal];
        providerString = [NSString stringWithFormat:@"%@", [[providerArray objectAtIndex:row] objectForKey:@"Code"]];
    }
    if (pickerNumber == 2) {
...
-(void)setupArrays {
    ...
    //load array
    NSString *intsPath = [docsDirectory stringByAppendingPathComponent:@"providers.plist"];
    providerArray = [[NSMutableArray alloc] initWithContentsOfFile:intsPath];
...
- (IBAction)loadAdvanced:(id)sender {
    ...
    [institutions setTitle:@"Institutions" forState:UIControlStateNormal];
...
- (IBAction)institutionsSet:(id)sender {
    ...
    if (UIAccessibilityIsVoiceOverRunning(TRUE)) {
        ...
        accessPicker.providerArray = providerArray;
        accessPicker.pickerNumber = 1;
